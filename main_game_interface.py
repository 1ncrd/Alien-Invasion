...
class MainGameInterface(Interface):
    ...
    def handle_input(self) -> Interface:
        ...
        while self.game_logic.level_up > 0:
            ...
            cur_level = self.game_logic.exp.level - self.game_logic.level_up
...
class LevelUpInterface(Interface):
    ...
    def display(self):
        ...
        # check_mark_left = "-> "
        # # for index in range(len(self.skill_choices)):
        # #     text = self.skill_choices[index][0]
        # #
        # #     # if selected, render like -> xxx
        # #     if index == self.selected_item:
        # #         text = check_mark_left + text
        # #
        #     # topleft = render_multiline_text(self.surface, self.font, text, topleft)
